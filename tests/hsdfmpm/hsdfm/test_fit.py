...
class TestFitFunctions(unittest.TestCase):
    ...
    def test_make_residual(self):
        ...
        npt.assert_array_equal(
            res_fun(self.params + 1),
            residual(self.params + 1, self.y, model=self.model, loss_thresh=1e-3),
        )
    ...
    def test_jacobian(self):
        eps = 1e-4
        npt.assert_allclose(
            jacobian(self.params, self.y, eps=eps, model=self.model),
            BASIS,
            rtol=1e-2,
            atol=1e-2,
        )
    ...
    def test_make_jacobian(self):
        ...
        npt.assert_array_equal(
            jac_fun(self.params + 1),
            jacobian(self.params + 1, self.y, eps=1e-4, model=self.model),
        )
    ...
    def test_fit_voxel(self):
        params, score = fit_voxel(
            voxel=self.y, model=self.model, x0=[0, 0, 0], loss_thresh=1e-3
        )
        npt.assert_allclose(params, self.params, rtol=1e-7, atol=1e-9)
        chi_sq = (1 / (N_WAVEL - N_PARAM)) * np.sum(
            ((self.y - self.model(*params)) ** 2 / (np.var(self.y)))
        )
    ...
    def test_fit_volume(self):
        ...
        params, score = fit_volume(
            volume=volume, model=self.model, n_workers=2, x0=[0, 0, 0]
        )
