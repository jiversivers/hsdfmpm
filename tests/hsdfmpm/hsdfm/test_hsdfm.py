...
class TestHyperspectralImage(unittest.TestCase):
    def setUp(self):
        ...
        with patch(
            "hsdfmpm.hsdfm.hsdfm.read_metadata_json",
            side_effect=lambda *a, **kw: self.md_vals.copy(),
        ):
            with patch(
                "hsdfmpm.hsdfm.hsdfm.read_hyperstack",
                side_effect=lambda *a, **kw: self.std_arr.copy(),
            ):
                self.mock_std = MergedHyperspectralImage(
                    image_paths=["dummy/path2", "dummy/path2"]
                )
                self.mock_std = HyperspectralImage(image_path="dummy/path2")

            with patch(
                "hsdfmpm.hsdfm.hsdfm.read_hyperstack",
                side_effect=lambda *a, **kw: self.bg_arr.copy(),
            ):
                self.mock_bg = HyperspectralImage(image_path="dummy/path")

            with patch(
                "hsdfmpm.hsdfm.hsdfm.read_hyperstack",
                side_effect=lambda *a, **kw: self.hs_vals.copy(),
            ):
                self.hsi = HyperspectralImage(image_path="dummy_path")
                self.hsi_subset = HyperspectralImage(
                    image_path="dummy_path",
                    wavelengths=[
                        self.md_vals["Wavelength"][wl] for wl in self.sel_wl_idx
                    ],
                )
                self.hsi_with_scalar = HyperspectralImage(
                    image_path="dummy_path", scalar=self.scalar
                )
                self.hsi_with_array_scalar = HyperspectralImage(
                    image_path="dummy_path", scalar=self.hs_vals / self.scalar
                )
                self.hsi_with_standard = HyperspectralImage(
                    image_path="dummy_path",
    ...
    def test_load_data(self):
        ...
        self.assertEqual(
            self.hsi_subset.metadata["ExpTime"],
            [self.md_vals["ExpTime"][wl] for wl in self.sel_wl_idx],
        )
        self.assertEqual(
            self.hsi_subset.metadata["Wavelength"],
            [self.md_vals["Wavelength"][wl] for wl in self.sel_wl_idx],
        )
        ...
        # With scalar scalar
        self.assertEqual(
            self.hsi_with_scalar.metadata["ExpTime"], self.md_vals["ExpTime"]
        )
        npt.assert_array_equal(
            self.hsi_with_scalar._hyperstack, self.hs_vals / self.scalar
        )
        npt.assert_array_equal(
            self.hsi_with_scalar._active, self.hsi_with_scalar._hyperstack
        )

        # With array scalar
        self.assertEqual(
            self.hsi_with_array_scalar.metadata["ExpTime"], self.md_vals["ExpTime"]
        )
        npt.assert_array_equal(
            self.hsi_with_array_scalar._hyperstack,
            self.hs_vals / (self.hs_vals / self.scalar),
        )
        npt.assert_array_equal(
            self.hsi_with_array_scalar._active, self.hsi_with_array_scalar._hyperstack
        )
    ...
    def test_normalize_integration_time(self):
        ...
        self.assertTrue(
            np.all(
                self.hsi
                == self.hs_vals
                / np.array(self.md_vals["ExpTime"])[..., np.newaxis, np.newaxis]
            )
        )
    ...
    def test_normalize_to_standard(self):
        ...
        npt.assert_array_equal(
            self.hsi_with_standard,
            (self.hs_vals - self.bg_arr) / (self.std_arr - self.bg_arr),
        )

    def test_normalize(self):
        self.hsi_with_standard.normalize_integration_time = MagicMock(
            side_effect=self.hsi_with_standard.normalize_integration_time
        )
        self.hsi_with_standard.normalize_to_standard = MagicMock(
            side_effect=self.hsi_with_standard.normalize_to_standard
        )
    ...
    def test_persistence_when_nested(self):
        ...
        self.mock_std.normalize_integration_time()
        npt.assert_array_equal(self.hsi_with_standard.standard, self.mock_std)

    def test_normalize_with_missing(self):
        self.assertRaises(OpticalPropertyError, self.hsi.normalize_to_standard)
        self.assertRaises(OpticalPropertyError, self.hsi.normalize)
        self.hsi.normalize_integration_time = MagicMock(
            side_effect=self.hsi.normalize_integration_time
        )
        self.hsi.normalize_to_standard = MagicMock(
            side_effect=self.hsi.normalize_to_standard
        )
    ...
    def test_bin(self):
        ...
        npt.assert_array_almost_equal(
            np.mean(self.hsi, axis=(1, 2)), self.hs_vals.mean(axis=(1, 2))
        )
    ...
    def test_subset_by_metadata(self):
        ...
        npt.assert_array_equal(
            self.hsi.image,
            self.hs_vals[np.isin(self.md_vals["Wavelength"], np.arange(500, 520, 10))],
        )
    ...
    def test_fit(self):
        ...
        self.assertTrue(np.all(score < 1.5))
        npt.assert_allclose(
            np.mean(param_img, axis=(1, 2)),
            np.mean(self.bio_params, axis=(1, 2)),
            rtol=0.25,
            atol=2.5,
        )
    ...
    def test_apply_kernel_bank(self):
        ...
        for k in bank:
            conv = np.array(
                [
                    fftconvolve(hs.astype(np.float32), k[::-1, ::-1], mode="same")
                    for hs in self.hs_vals
                ]
            )
...
class TestMergedHyperspectralImage(unittest.TestCase):
    def setUp(self):
        ...
        with patch("hsdfmpm.hsdfm.hsdfm.read_metadata_json", return_value=self.md_vals):
            with patch("hsdfmpm.hsdfm.hsdfm.read_hyperstack") as hyperstacker:
                ...
                self.mhsi = MergedHyperspectralImage(
                    image_paths=["dummy/path2", "dummy/path2", "dummy/path3"],
                    metadata_path=self.md_path,
                )
    ...
    def test_averagable_attr(self):
        npt.assert_array_equal(
            self.mhsi.hyperstack, np.mean(self.expected_list, axis=0)
        )
