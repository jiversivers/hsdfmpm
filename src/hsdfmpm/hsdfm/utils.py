...
def normalize_integration_time(hyperstack: np.ndarray[float], integration_time: list[float]) -> np.ndarray[float]:
    hyperstack /= np.array(integration_time)[:, np.newaxis, np.newaxis]
    return hyperstack

def normalize_to_standard(hyperstack: np.ndarray[float], standard: np.ndarray[float], bg: np.ndarray[float]):
    return (hyperstack - bg) / (standard - bg)

def get_local_stdev(image: np.ndarray[float], shape: tuple[int, int]) -> np.ndarray[float]:
    C, H, W = image.shape
...
def mask_by_k_clustering(src: np.ndarray[float], ks: list[int], **kwargs) -> np.ndarray[bool]:
    ...
    # Collapse to mask
    mask = slice_clusters(src, clusters, slice_to_take=kwargs['slice_to_take'] if 'slice_to_take' in kwargs else None)

    return mask

def k_cluster(src: np.ndarray[float], k: int = 3, include_location: bool = False) -> np.ndarray[int]:
...
def intra_vs_inter_cluster_variance(src: np.ndarray[float], labels: np.ndarray[int]) -> np.float64:
    if src.ndim > 2:
...
def try_n_clusters(src: np.ndarray[float], ks: list[int], **kwargs) -> tuple[np.ndarray[bool], np.ndarray[float]]:
    # KMeans Clustering
    clusters = np.zeros((len(ks),) + src.shape[-2:])
    scores = np.zeros(len(ks))
    for i, k in enumerate(ks):
        clusters[i] = k_cluster(src, k, include_location=kwargs['include_location'] if 'include_location' in kwargs else False)
        scores[i] = intra_vs_inter_cluster_variance(src, clusters[i])
    return clusters, scores

def find_elbow_clusters(clusters: np.ndarray[int], scores: np.ndarray[float]) -> tuple[np.ndarray[int], int]:
...
def slice_clusters(src: np.ndarray[float], clusters: np.ndarray[bool], slice_to_take: slice = None
                   ) -> np.ndarray[bool]:
...
def try_n_gaussians(src: np.ndarray[float], ks: list[int], **kwargs) -> tuple[np.ndarray[bool], np.ndarray[float]]:
    # Gaussian Mixture Modelling
    clusters = np.zeros((len(ks),) + src.shape[-2:])
    scores = np.zeros(len(ks))
    for i, k in enumerate(ks):
        clusters[i] = gm_model(src, k,
                                include_location=kwargs['include_location'] if 'include_location' in kwargs else False)
        scores[i] = intra_vs_inter_cluster_variance(src, clusters[i])
    return clusters, scores

def gm_model(src: np.ndarray[float], k: int = 3, include_location: bool = False) -> np.ndarray[int]:
    X = prepare_clustering_variables(src, include_location=include_location)
    labels = GaussianMixture(n_components=k, random_state=42).fit_predict(X)
    return labels.reshape(src.shape[-2:])

def prepare_clustering_variables(src: np.ndarray[float], include_location: bool = False) -> tuple[np.ndarray[float], np.ndarray[int]]:
    shape = src.shape[-2:]
...
def leastsq_reflectance(hyperstack: np.ndarray[float], eps: np.ndarray[float]
    ...
    # Fit each pixel
    for i, j in itertools.product(range(hyperstack.shape[1]), range(hyperstack.shape[2])):
